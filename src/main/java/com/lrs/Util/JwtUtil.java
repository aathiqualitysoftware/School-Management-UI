...
public class JwtUtil {
    ...
    public String generateToken(String username, List<String> roles, StudentDetailsProjection students) {
        ...
        List<String> teachingstfcnt = roles.stream().filter(x->x.equalsIgnoreCase("Teaching Staff")).toList();
        List<String> parentcnt = roles.stream().filter(x->x.equalsIgnoreCase("Parent")).toList();

        if(!cntList.isEmpty() || !teachingstfcnt.isEmpty() || !parentcnt.isEmpty()){
